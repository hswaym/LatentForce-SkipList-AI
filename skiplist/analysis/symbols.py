...
class ScopedSymbolVisitor(ast.NodeVisitor):
    ...
    def _get_line_start(self, node: ast.AST) -> int:
        return getattr(node, "lineno", 1)

    def _get_line_end(self, node: ast.AST) -> int:
        end = getattr(node, "end_lineno", None)
        line_start = getattr(node, "lineno", 1)
        if end is not None and isinstance(end, int) and end >= line_start:
            return end
        return line_start

    def visit_ClassDef(self, node: ast.ClassDef):
        qual_name = self._current_qual_name(node.name)
        line_start = self._get_line_start(node)
    ...
    def _handle_func(self, node: ast.FunctionDef | ast.AsyncFunctionDef):
        ...
        kind = "method" if (self.scope and self.scope[-1][1]) else "function"
